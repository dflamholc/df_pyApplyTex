...
def checkTags(obj, objName, matList, matNames):
    ...
    if objTEX and not objUVW in tags:
        ...
        while True: # make tag iteration function from this (see if tempTag list can be used for other places where we iterate on tags)
            if tag == objTEX:
                checkTag.append(tag)
            if not tag.GetNext():
                break
        ...
        proj = 'uvw'
        placeTag(obj, objName, matList, matNames, proj) # if there is no Tex Tag but UVW tag place tag - UVW
